...
func TestSetupCircuitFromFile(t *testing.T) {
	nContributionsPhase1 := 1
	power := 9 + Cnt
	contributionsPhase1 := make([]phase1.Contribution, nContributionsPhase1)
	tCheck := time.Now()
	fmt.Println("Phase1 init", tCheck)
	contributionsPhase1[0].Initialize(power)
	fmt.Println("Phase1 init took", time.Since(tCheck))
	...
	{
		fmt.Println("r1cs gob dump")
		// r1csDump, _ := os.Create("r1cs.gob.dump")
		// writer := bufio.NewWriter(r1csDump)
		// enc := gob.NewEncoder(writer)
		// enc.Encode(ccs)
		// r1csDump.Close()
		utils.SplitDumpR1CSBinary(ccs.(*cs_bn254.R1CS), "Foo", 10000)
	}
	...
	// evals = contributionsPhase2[0].PreparePhase2(&contributionsPhase1[nContributionsPhase1-1], r1cs)
	tCheck = time.Now()
	fmt.Println("Phase2 init", tCheck)
	evals = contributionsPhase2[0].PreparePhase2FromFile(&contributionsPhase1[nContributionsPhase1-1], "Foo", N, batchSize)
	fmt.Println("Phase2 init took", time.Since(tCheck))
...
func TestFoo(t *testing.T) {
	tCheck := time.Now()
	domain := fft.NewDomain(1 << 26)
	coefs := make([]bn254.G1Affine, domain.Cardinality)
	for i, _ := range coefs {
		var c bn254.G1Affine
		c.ScalarMultiplicationBase(big.NewInt(int64(i)))
		coefs[i].Set(&c)
	}
	fmt.Println(domain.Cardinality, time.Since(tCheck))
	tCheck = time.Now()
	numCPU := uint64(runtime.NumCPU())
	maxSplits := bits.TrailingZeros64(ecc.NextPowerOfTwo(numCPU))

	utils.DifFFTG1(coefs, domain.TwiddlesInv, 0, maxSplits, nil)
